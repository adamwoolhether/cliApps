		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				if !errors.Is(err, expError) {
					t.Fatalf("exp err %q, got %q", expError, err)
				}
			}
			if err == nil {
				t.Errorf("exp error %q, got nil", expError)
			}
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			start := func(i pomodoro.Interval) {
				if i.State != pomodoro.StateRunning {
					t.Errorf("exp state %d, got %d", pomodoro.StateRunning, i.State)
				}
				if i.ActualDuration >= i.PlannedDuration {
					t.Errorf("exp ActualDuraction %q, less than Planned %q.\n", i.ActualDuration, i.PlannedDuration)
				}
			}
			
			end := func(i pomodoro.Interval) {
				if i.State != tc.expState {
					t.Errorf("exp state %d, got %d", tc.expState, i.State)
				}
				if tc.cancel {
					t.Errorf("end callback should not be executed.")
				}
			...
			if i.ActualDuration != tc.expDuration {
				t.Errorf("exo ActualDuration %q, got %q", tc.expDuration, i.ActualDuration)
			}
			cancel()
		})
	}
