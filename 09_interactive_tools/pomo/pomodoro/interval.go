...
// Interval represents the pomodo interval
...
// Repository abstracts data sources and defines
// to conduct actions on Interval.
...
// IntervalConfig represents configuration required by an Interval.
...
// NewConfig instantiates a new IntervalConfig with the provided values.
...
// nextCategory takes a reference to a repository
// and returns the next interval category as a string or error.
...
// tick controls the timeer for each interval's execution.
// It executes actions every second that the interval progresses.
...
// /////////
// Define the API for the Interval type.

// GetInterval takes a intervalConfig and returns
// an instance of the Interval type or error.
...
// Start is used to start the interval. It checks the state of the
// current interval, sets the options, and times the interval with tick().
...
// Pause is used to pause a running interval.
