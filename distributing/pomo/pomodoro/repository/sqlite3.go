...
// NewSQLite3Repo returns a new *dbRepo with basic configuration
// and initial 'interval' table seeded.
// It creates a new .db file is the given name doesn't exist.
// For more advanced configuration, see: https://github.com/mattn/go-sqlite3#connection-string
...
func (r *dbRepo) Create(i pomodoro.Interval) (int64, error) {
	// Create an entry in the repo
	r.Lock()
	defer r.Unlock()

	// Prepare INSERT statement
	stmt, err := r.db.Prepare("INSERT INTO interval VALUES(NULL, ?,?,?,?,?)")
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
	...
	res, err := stmt.Exec(i.StartTime, i.PlannedDuration, i.ActualDuration, i.Category, i.State)
	if err != nil {
		return 0, err
	}
...
func (r *dbRepo) Update(i pomodoro.Interval) error {
	...
	stmt, err := r.db.Prepare("UPDATE interval SET start_time=?, actual_duration=?, state=? WHERE id=?")
	if err != nil {
		return err
	}
	defer stmt.Close()

	res, err := stmt.Exec(i.StartTime, i.ActualDuration, i.State, i.ID)
	if err != nil {
		return err
	}

	// UDATE results and check for errors.
