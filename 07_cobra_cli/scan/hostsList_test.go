		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectErr != nil {
				...
				if !errors.Is(err, tc.expectErr) {
					t.Errorf("Expected error %q, go %q", tc.expectErr, err)
				}
			...
			if hl.Hosts[1] != tc.host {
				t.Errorf("Expected host name %q as index 1, got %q.", tc.host, hl.Hosts[1])
			}
		})
	}
}
...
func TestRemove(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err != nil {
				t.Fatalf("Exptected no error, got %q instead\n", err)
			}
			if len(hl.Hosts) != tc.expectLen {
