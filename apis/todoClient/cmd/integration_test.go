...
// gotest -v ./cmd -tags integration -count=1
...
func TestIntegration(t *testing.T) {
	...
	t.Run("ListTasks", func(t *testing.T) {
		...
		if taskCompleteStatus != "-" {
			t.Errorf("exp status %q, got %q", "-", taskCompleteStatus)
		}
	...
	vRes := t.Run("ViewTask", func(t *testing.T) {
		...
		if !strings.Contains(viewOut[0], task) {
			t.Fatalf("exp task %q, got %q", task, viewOut[0])
		}
		if !strings.Contains(viewOut[1], today) {
			t.Fatalf("exp creation day/month %q, got %q", today, viewOut[1])
		}
		if !strings.Contains(viewOut[2], "No") {
			t.Fatalf("exp completed status %q, got %q", "No", viewOut[2])
		}
	})
	...
	t.Run("CompleteTask", func(t *testing.T) {
		...
		expOut := fmt.Sprintf("Item number %s marked as complete.\n", taskID)
		if expOut != out.String() {
			t.Fatalf("exp output %q, got %q", expOut, out.String())
		}
	})
	...
	t.Run("ListCompletedTask", func(t *testing.T) {
		...
		if taskCompleteStatus != "X" {
			t.Errorf("exp status %q, got %q", "X", taskCompleteStatus)
		}
	})
