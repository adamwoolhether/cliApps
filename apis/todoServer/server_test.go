...
import (
	...
	"github.com/adamwoolhether/cliApps/interacting/todo"
)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if r.StatusCode != tc.expCode {
				t.Fatalf("Exp code %q, got %q", http.StatusText(tc.expCode), http.StatusText(r.StatusCode))
			}
			...
			case r.Header.Get("Content-Type") == "application/json":
				...
				if resp.TotalResults != tc.expItems {
					t.Errorf("Exp %d items, got %d", tc.expItems, resp.TotalResults)
				}
				if resp.Results[0].Task != tc.expContent {
					t.Errorf("Exp %q, got %q", tc.expContent, resp.Results[0].Task)
				}
			case strings.Contains(r.Header.Get("Content-Type"), "text/plain"):
				...
				if !strings.Contains(string(body), tc.expContent) {
					t.Errorf("Exp %q, got %q", tc.expContent, string(body))
				}
			default:
...
func TestAdd(t *testing.T) {
	...
	t.Run("Add", func(t *testing.T) {
		...
		if r.StatusCode != http.StatusCreated {
			t.Errorf("Exp code %q, got %q", http.StatusText(http.StatusCreated), http.StatusText(r.StatusCode))
		}
	})
	...
	t.Run("CheckAdd", func(t *testing.T) {
		...
		if r.StatusCode != http.StatusOK {
			t.Fatalf("Exp %q, got %q", http.StatusOK, r.StatusCode)
		}
		...
		if resp.Results[0].Task != taskName {
			t.Errorf("Exp %q, got %q", taskName, resp.Results[0].Task)
		}
	})
}
...
func TestDelete(t *testing.T) {
	...
	t.Run("Delete", func(t *testing.T) {
		...
		if r.StatusCode != http.StatusNoContent {
			t.Fatalf("Exp %q, got %q", http.StatusText(http.StatusNoContent), http.StatusText(r.StatusCode))
		}
	})
	...
	t.Run("CheckDelete", func(t *testing.T) {
		...
		if r.StatusCode != http.StatusOK {
			t.Fatalf("Exp %q, got %q", http.StatusText(http.StatusOK), http.StatusText(r.StatusCode))
		}
		...
		if len(resp.Results) != 1 {
			t.Errorf("Exp 1 item, got %d", len(resp.Results))
		}
		expTask := "Task number 2"
		if resp.Results[0].Task != expTask {
			t.Errorf("Exp %q, got %q", expTask, resp.Results[0].Task)
		}
	})
}
...
func TestComplete(t *testing.T) {
	...
	t.Run("CheckComplete", func(t *testing.T) {
		...
		if len(resp.Results) != 2 {
			t.Errorf("Exp 2 items, got %d", len(resp.Results))
		}
		if !resp.Results[0].Done {
			t.Error("Exp Item 1 to be completed")
		}
		if resp.Results[1].Done {
			t.Error("Exp Item 2 not to be completed")
		}
	})
}
